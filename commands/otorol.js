...
exports.run = async (client, message, args) => {
  ...
  if(message.author.id !== "KENDİ İD NİZ") {return message.channel.send('Bot Şuanda Bakımda!')}
    ...
    if(args[0] === 'rol-ayarla') {
        ...
        message.channel.send(`:white_check_mark: Harikasın ! Otorol başarılı bir şekilde ${rol} olarak ayarlandı`)
        ...
        if(!kanal) return message.channel.send('Bir kanal etiketlemediniz'); else {
            ...
            message.channel.send(`:white_check_mark: Harikasın ! Otorol kanal başarılı bir şekilde ${kanal} olarak ayarlandı`)
        }
